...
public class MainGameController : MonoBehaviour {
    ...
    public int customersToBeServed = 10;		
	// Static Variables //
	//******************//
	static public string gameMode;			//game mode setting (freeplay or career)
	static public int gameTime;
	static public int requiredCustomers;
	//*******************

	//public game objects
	public GameObject[] customers;	//list of all available customers (different textures)
     ...
     * 
    */
    public Dictionary<int, int> inventory = new Dictionary<int, int>(); //key: factory id, value: amount
    public TextMeshProUGUI[] itemAmountTexts; //should be exactly same size as the inventory
    bool inventoryDone; //flag
    public GameObject inventoryPanel;
    public TextMeshProUGUI[] panelItemAmountTexts;
    ...
    static public int totalCustomersServed;
    ...
    void InitInventory() {
        ...
        if (inventory != null)
        {
            ...
            {
                inventory.Add(i + 1, 0);
                itemAmountTexts[i].text = " x 0";
                panelItemAmountTexts[i].text = " x 0";
            }
        }
        inventoryDone = false;
	...
	void InitGame (){
		...
		switch(gameMode) {
			case "FREEPLAY":
                requiredCustomers = customersToBeServed;
				gameTime = 0;
				//canUseCandy = true;
				break;
			/*case "CAREER":
    ...
    public void FinishInventory(){
        ...
        foreach (var item in inventory){
            itemAmountTexts[item.Key - 1].text = " x " + item.Value.ToString();
        }
        inventoryPanel.SetActive(false);
        decreaseMoneyToBuyIngredients();
    }

    void decreaseMoneyToBuyIngredients()
    {
        int sum = 0;
        foreach (var ingredientObj in IngredientsController.ingredientsArray)
        {
            sum += inventory[ingredientObj.GetComponent<IngredientsController>().factoryID] * ingredientObj.GetComponent<IngredientsController>().price ;
        }
        int currMoney = PlayerPrefs.GetInt("PlayerMoney");
        currMoney -= sum;
        PlayerPrefs.SetInt("PlayerMoney", currMoney);
    }
    //***************************************************************************//
    // FSM
    //***************************************************************************//
    ...
    public void serveCurrentCustomer(){
        if (currentCustomer) {
            currentCustomer.GetComponent<CustomerController>().fulfillOrder();
        }
    }
	
    public void decreaseInventory(int[] items) {
        ...
        {
            itemAmountTexts[item.Key - 1].text = " x " + inventory[item.Key].ToString();
        }

    }

    public void addInventoryItem(int factoryID) {
        inventory[factoryID] += 1;
        panelItemAmountTexts[factoryID - 1].text = " x " + inventory[factoryID].ToString();
    }

    public void deleteInventoryItem(int factoryId){
        if (inventory[factoryId] > 0)
            inventory[factoryId] -= 1;
        panelItemAmountTexts[factoryId - 1].text = " x " + inventory[factoryId].ToString();
    }
	...
	void manageGuiTexts (){
        ...
        missionText.GetComponent<TextMesh>().text =  totalCustomersServed.ToString() + " Customers";
	}
	...
	IEnumerator processGameFinish (){
		...
		//did we reached the level goal? always reach the goal to serve 10 customers
        if(totalCustomersServed >= requiredCustomers) {
	...
	IEnumerator checkGameWinState (){
		...
		/*if(gameMode == "CAREER" && gameTime <= 0 && totalMoneyMade < requiredBalance) {
		...
		} else*/ if(gameMode == "FREEPLAY" && totalCustomersServed >= requiredCustomers) {
		
			print("Wow, You beat the goal. But You can continue... :)");
			playNormalSfx(winSfx);
            endGamePlane.SetActive(true);
            endGameStatus.GetComponent<Renderer>().material.mainTexture = endGameTextures[0];
            gameIsFinished = true;
            ...
            PlayerPrefs.SetInt("PlayerMoney", currentMoney);
            int currDay = PlayerPrefs.GetInt("BusinessDay");
            currDay += 1;
            PlayerPrefs.SetInt("BusinessDay", currDay);
        }
	}
